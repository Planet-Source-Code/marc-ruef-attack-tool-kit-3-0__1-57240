...
Private Sub Form_Load()
    ...
    frmSplashScreen.lblStatusInformation.Caption = "finishing the loading sequence"
...
Private Sub CheckDirectoriesBeforeLoading()
    ...
    WriteLogEntry "Checking the existence of the suggestions directory " & SuggestionsDirectory & " ...", 6
    If Not (Dir$(SuggestionsDirectory, 16) <> "") Then
        Call errSuggestionsDirectoryNotExist
    End If

    'Check the existence of the logs directory
    WriteLogEntry "Checking the existence of the logs directory " & LogsDirectory & " ...", 6
    If Not (Dir$(LogsDirectory, 16) <> "") Then
        Call errLogDirectoryNotExist
    End If

    'Check the existence of the reports directory
    'WriteLogEntry "Checking the existence of the reports directory " & ReportsDirectory & " ...", 6
    'If Not (Dir$(ReportsDirectory, 16) <> "") Then
    '   Call errReportDirectoryNotExist
    'End If
End Sub
...
Private Sub AttackProcedure()
    ...
    If InStr(1, session_procedure_commands, "$DHOST", vbBinaryCompare) Then
        session_procedure_commands = Replace(session_procedure_commands, "$DHOST", Target, , , vbBinaryCompare)
    End If
    
    If InStr(1, session_procedure_commands, "$DPORT", vbBinaryCompare) Then
    ...
    For i = 0 To CommandCount
        ...
        ElseIf Mid$(Command(i), 1, 4) = "send" Then
            ...
            If wskTCPWinsock(0).State = 7 Then
                ...
                WriteLogEntry "Sending data ..." & DataToSend, 6
        ...
        ElseIf Mid$(Command(i), 1, 3) = "run" Then
            ...
            strRunCommandFileName = ResponseDirectory & Target & "-runcommandresponse.txt"
            
            'run the selected command
            Shell Environ("Comspec") + " /C " & strRunCommand & " > " & strRunCommandFileName, vbMinimizedNoFocus
            
            'wait until the command is finished
            Pause (DefaultSleepValue / 1000)
...
Public Sub LoadATKPlugins()
    ...
    If (Dir$(PluginDirectory, 16) <> "") Then
        ...
        If ListCountOfATKPlugins <> 0 Then
            ...
            WriteLogEntry "Loading the plugins from " & PluginDirectory & ".", 6
...
Private Sub mnuPluginsShowOnlinePluginItem_Click()
    If LenB(plugin_filename) Then
        ...
        WebSiteURL = PluginsDownloadURL & plugin_filename & ".html"
...
Private Sub txtPluginContent_DblClick()
    ...
    strSelectedText = txtPluginContent.SelText
...
Private Sub wskTCPWinsock_DataArrival(Index As Integer, ByVal bytesTotal As Long)
    ...
    WriteLogEntry "Receiving data from the target ...", 6
...
Private Sub SearchPlugin()
    ...
    WriteLogEntry "Starting the search for the string " & strPluginSearchText, 1
